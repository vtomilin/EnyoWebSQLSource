...
(function(enyo) {
    ...
    function failTemplate(rec, opts, e) {
        if(opts && typeof opts.fail === 'function') {
            ...
            this.error('WebSQL operation failed for record:', rec,
    ...
    enyo.kind({
        ...
        constructor: enyo.inherit(function(sup) {
            return function(props) {
                ...
                if(props.dbSchema) {
                    websql.Source.updateSchema(this._db,
                                               props.dbSchema,
                                               props.dbSchemaDidUpdate);
                } else {
                    enyo.Signals.send('onWebSQLDbReady');
                }
        ...
        fetch: function (rec, opts) {
            var o = enyo.clone(opts) || {},
                sql = websql.Source.buildSelect(rec, o),
        ...
        commit: function (rec, opts) {
            var o = enyo.clone(opts) || {},
                sql = rec.isNew ? websql.Source.buildInsert(rec, o)
                                : websql.Source.buildUpdate(rec, o),
                fail = failTemplate.bind(this, rec, opts);
        ...
        destroy: function (rec, opts) {
            var o = enyo.clone(opts) || {},
                sql = websql.Source.buildDelete(rec, o),
                fail = failTemplate.bind(this, rec, opts);
        ...
        /**
         Runs arbitrary SQL with optional arguments given. Invokes specified
         callback once done. The callback is invoked with an error object if
         error has occured. Otherwise, first argument will be undefined, second
         will contain SQLResultSet object. Returns this.
         
        *Arguments:*
         _sql_ SQL statement to execute
         _args_ Optinal arguments to the SQL statement, provided the statement
         contained '?' templates.
         _callback_ Callback function to be invoked with two arguments(undefined,
         SQLresultSet) if the statement execution was successful, and one
         argument(error object) if it failed.
         */
        execSql: function(sql, args, callback) {
            var hdr = '[WebSQLSource.execSql]';
            if(this._debug) {
                this.log(hdr, sql, args, callback);
            }
            
            this._db.transaction(function(t) {
                var cb = typeof callback === 'function' ? callback
                    : function(e, r) {
                        enyo.error(hdr, 'executed statement', sql, 'Error:', e,
                                   'Result:', r);
                    };
        ...
        statics: {
            ...
            buildSelect: function(rec, opts) {
                var schema = websql.Source.extractSchema(rec, opts),
                    id,
